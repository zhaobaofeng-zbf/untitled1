...
# cv2.imwrite("gray.png", gray, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])
...
binary = gray
cv2.imwrite("binary.png", binary, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])
    ...
    while True:
        ...
        if math.fabs(cv2.contourArea(approx)) > Config.min_area:
            if (len(approx) > 4):
                ...
                cv2.imwrite("transfering%d.png"%idx, warped, [int(cv2.IMWRITE_PNG_COMPRESSION), 9])
                break
        else:
